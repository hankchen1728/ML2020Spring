...
def main(args):
    ...
    WG = WGenerator(args.z_dim).to(device)
    WG.load_state_dict(torch.load(args.gen_ckpt))
    WG.eval()

    same_seeds(10)
    n_output = 20
    z_sample = Variable(torch.randn(n_output, args.z_dim)).to(device)
    imgs_sample = (WG(z_sample).data + 1) / 2.0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
            "--gen_ckpt",
            type=str,
            default="./checkpoints/WGAN/wgan_g.pth")

    parser.add_argument(
            "--save_img",
            type=str,
            default="./image_rep/p2.png")
